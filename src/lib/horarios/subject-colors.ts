/**
 * subject-colors.ts â€” Sistema Unificado de Paletas y Colores DeterminÃ­sticos por Materia
 * SIGPDA-EMS Â· EducaciÃ³n Media Superior
 *
 * Garantiza que la misma materia tenga exactamente los mismos colores armÃ³nicos
 * en PDF Oficial, Word Editable (.docx), Excel Estilizado (ExcelJS) y Tarjeta WhatsApp (NeÃ³n).
 */
...
// 9 Pares determinÃ­sticos (Contrastes AA verificados)
const PALETTE_SUBJECTS = [
  ...
  { pastelBg: "FEF3C7", pastelText: "92400E", neon: "#FDE047", border: "#FDE68A" }, // Ãmbar / Miel
  { pastelBg: "FCE7F3", pastelText: "9D174D", neon: "#F472B6", border: "#FBCFE8" }, // Rosa / Magenta
  { pastelBg: "E0E7FF", pastelText: "3730A3", neon: "#C084FC", border: "#C7D2FE" }, // Ãndigo / Lavanda
  { pastelBg: "FFEDD5", pastelText: "9A3412", neon: "#FB923C", border: "#FED7AA" }, // Naranja Suave
  { pastelBg: "F3E8FF", pastelText: "6B21A8", neon: "#E879F9", border: "#E9D5FF" }, // PÃºrpura
  { pastelBg: "CCFBF1", pastelText: "115E59", neon: "#2DD4BF", border: "#99F6E4" }, // Menta / Turquesa
  { pastelBg: "FEE2E2", pastelText: "991B1B", neon: "#F87171", border: "#FECACA" }  // Coral / SalmÃ³n
];
...
/**
 * Obtiene los colores determinÃ­sticos de una materia a partir de su nombre.
 */
export function getSubjectColors(materiaNombre?: string): SubjectColor {
  if (!materiaNombre || materiaNombre.trim().toLowerCase() === "libre" || materiaNombre.trim() === "â€”" || materiaNombre.trim() === "") {
    return COLOR_LIBRE;
  }
